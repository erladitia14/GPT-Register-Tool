"""
Ekstraksi tautan redirect Kakao Pay / Nicepay.

File ini dikelola terpisah, tidak bergantung pada skrip iDEAL atau BLIK utama:
  proxy_seeds.txt berisi satu sticky Seed per baris
  -> Derivasikan region Checkout / Bootstrap Stripe init dari Seed yang sama
  -> region checkout/update
  -> region Stripe refresh / taxes / Kakao / approve / redirect

Seed gagal tidak dicoba lagi dalam tugas yang sama; status antar tugas disimpan di
proxy_state.json direktori ini. Sukses akan reset hitungan gagal; error proxy yang jelas akan hapus Seed.
"""
...
# Proxy Sticky (seperti cliproxy) ditentukan oleh sid session: sid yang sama akan menempel pada IP yang sama dalam TTL,
# sehingga derivasi region berbeda terjebak pada exit yang sama. Agar checkout/provider(KR) dan
# promotion(VN) mendapatkan exit lokal masing-masing, Kakao membuat sid terpisah untuk tiap region (tambahkan suffix negara).
...
def proxy_chain_key(proxy: str) -> str:
    ...
    # Hapus sufiks kode negara 2 huruf kapital di akhir sid (ditambahkan oleh proxy_for_country),
    # pertahankan sid base saja. Seed turunan tiga negara (base+KR/base+VN) dari base yang sama dinormalisasi
    # ke base yang sama → chain_key sama (sticky check passed); sedangkan Seed redundant dengan
    # hanya base sid yang berbeda tetap menjadi chain_key berbeda, tidak akan dideduplikasi oleh load_proxy_seeds.
    # Diasumsikan base sid tidak diakhiri dengan 2 huruf kapital berurutan.
...
def proxy_for_country(proxy: str, country: str) -> str:
    ...
    # Tambahkan akhiran wilayah ke sid, agar proksi sticky menetapkan IP keluar independen untuk setiap wilayah. Selalu panggil dari Seed asli
    # (seperti yang dilakukan kakao_proxy_chain), hindari akumulasi akhiran. Proksi tanpa sid tidak terpengaruh.
...
def kakao_proxy_chain(proxy_seed: str) -> tuple[str, str, str]:
    ...
    if not key or any(
        ...
        raise RuntimeError("Pengubahan wilayah proxy mengubah sticky Seed, menolak penggunaan rantai proxy campuran")
...
def load_proxy_seeds() -> list[str]:
    ...
    for proxy in unique:
        ...
        usable.append(proxy)
    if not usable:
        raise RuntimeError("Semua Proxy Seed dalam masa pendingin gagal atau telah dihapus")
    ...
    log(f"Memuat {len(usable)} biji agen, lewati {skipped} biji pending/hapus")
    log(
        "Alur Wilayah: Satu Seed turunkan "
...
def ip_info(proxy: str) -> dict[str, str]:
    ...
    raise RuntimeError("Gagal kueri IP keluar:" + "；".join(failures[:4]))
...
def select_verified_seed(
    ...
    while True:
        ...
        try:
            ...
            state_text = "Dihapus" if state == "removed" else ("Masuk cooling" if state == "cooling" else "Pertahankan")
            log(f"Kakao Seed {proxy_label(proxy_seed)} tidak dapat diturunkan, {state_text}: {reason[:180]}", "[WARN] ")
            continue
        ...
        for role, proxy in (
            ...
            if ok:
                log(f"{role_label(role)} {proxy_label(proxy)} pra-periksa keluar lolos: {detail}")
                continue
            preflight_error = f"{role_label(role)} pra-periksa keluar gagal: {detail}"
            break
...
def update_checkout_promotion(session: Any, token: str, checkout_id: str, checkout: dict[str, Any]) -> None:
    ...
    log(f"{PROMOTION_COUNTRY} checkout/update berhasil: promo={promo_id if 'promo_campaign' in body else 'off'}")
...
def update_kakao_checkout_taxes(
    ...
    log(f"{PROVIDER_COUNTRY} sinkronisasi checkout/taxes berhasil")
...
def stripe_update_kakao_tax_region(
    ...
    log(f"{PROVIDER_COUNTRY} Stripe tax_region sinkronisasi berhasil: {billing['city']} {billing['postal_code']}")
...
def ensure_running(stop_event: Event | None) -> None:
    if stop_event is not None and stop_event.is_set():
        raise TaskStopped("Tugas Telah Dihentikan")
...
def kakao_link(
    ...
    *,
    ...
    log("Validasi Token ChatGPT")
    validate_kakao_access_token(token, checkout_proxy)

    ensure_running(stop_event)
    log(f"{CHECKOUT_COUNTRY} membuat checkout trial KRW Kakao")
    ...
    ensure_running(stop_event)
    log(f"{PROMOTION_COUNTRY} checkout/update lalu refresh Stripe melalui {PROVIDER_COUNTRY}")
    ...
    amount = inspect_kakao_init(
        init_payload, f"{PROMOTION_COUNTRY} setelah update {PROVIDER_COUNTRY}", require_zero=True
    )

    billing = random_kakao_billing(token)
    tax_elements_session_id = f"elements_session_{uuid.uuid4().hex[:11]}"
    ensure_running(stop_event)
    log(f"Sinkronkan checkout/taxes {PROVIDER_COUNTRY} dengan Stripe tax_region")
    ...
    ensure_running(stop_event)
    log(f"{PROVIDER_COUNTRY} refresh Stripe setelah sinkronisasi pajak")
    init_payload, stripe_js_id = stripe_init(provider_session, checkout_id, publishable_key, checkout_page)
    amount = inspect_kakao_init(init_payload, f"{PROVIDER_COUNTRY} sinkronisasi pajak", require_zero=True)
    ...
    ensure_running(stop_event)
    log(f"{PROVIDER_COUNTRY} membuat payment_method Kakao")
    ...
    if not redirect and (
        ...
        log(f"{PROVIDER_COUNTRY} OpenAI approve (maks. {APPROVE_RETRY_MAX} kali)")
        last_error = ""
            ...
            if approval_response.status_code == 200:
                try:
                    if (approval_response.json() or {}).get("result") == "approved":
                        log(f"{PROVIDER_COUNTRY} approve percobaan ke-{index} berhasil")
                        last_error = ""
                        break
                except (TypeError, ValueError):
                    pass
    ...
    log(f"{PROVIDER_COUNTRY} polling Stripe redirect (maks. {POLL_TIMEOUT}s)")
...
def run_single_seed_mode(token: str, proxy_seeds: list[str]) -> int:
    ...
    log(
        "Mulai eksekusi alur Seed tunggal Kakao:"
        ...
        f"Jumlah percobaan Seed per ronde={seeds_per_round}, ronde ulang={max_rounds},"
        f"Maksimal rantai lengkap={max_attempts}（{seeds_per_round} × {max_rounds}）。"
    )
    while attempt < max_attempts:
        candidate = select_verified_seed(proxy_seeds, attempted_keys)
        if candidate is None:
            last_error = (
                f"Tidak tersedia {CHECKOUT_COUNTRY} -> {PROMOTION_COUNTRY} -> "
                f"{PROVIDER_COUNTRY} proxy Seed"
            )
            break
        proxy_seed, checkout_proxy, promotion_proxy, provider_proxy = candidate
        attempt += 1
        log(
            f"Alur Lengkap {attempt}/{max_attempts}:"
        ...
        try:
            ...
            log("Tautan alih Kakao/Nicepay telah diperoleh")
            ...
            log("Tugas Telah Dihentikan", "[WARN] ")
            ...
            if is_account_error(error):
                log(f"Akun tidak dapat dilanjutkan: {error[:240]}", "[ERROR] ")
            ...
            if is_checkout_shape_error(error):
                log(
                    "Checkout Kakao untuk Seed saat ini tidak mempertahankan metode pembayaran atau 0 KRW;"
                    "Checkout dan Seed kali ini telah ditinggalkan, tidak dihitung sebagai kesalahan agen,"
                    f"Seed berikutnya akan membangun ulang {CHECKOUT_COUNTRY} -> {PROMOTION_COUNTRY} -> "
                    f"{PROVIDER_COUNTRY} rantai: {error[:260]}",
                    "[WARN] ",
                )
            else:
                log(
                    f"Jalur Seed Tunggal Kakao gagal, {state_text}; tugas ini tidak akan menggambar ulang Seed ini: {error[:260]}",
                    "[WARN] ",
                )

    log(f"Semua gagal: {last_error or 'Tidak dapat mendapatkan tautan pengalihan Kakao/Nicepay'}", "[ERROR] ")
...
def main() -> int:
    ...
    if not token:
        ...
        return 1
    log(f"Menggunakan {token_account(token)}")
    try:
