...
def stripe_init_pix(stripe: requests.Session, cs_id: str, stripe_pk: str) -> dict[str, Any]:
    """Init khusus PIX: tetap pt-BR + America/Sao_Paulo."""
    body = {
...
def create_pix_checkout(access_token: str, proxy_url: str = "", with_promo: bool = False) -> dict[str, Any]:
    """Checkout khusus PIX: default tidak dengan promo awal (konsisten dengan opll2)."""
...
def generate_valid_cpf() -> str:
    """Hasilkan CPF Brazil yang valid (termasuk digit pemeriksa) untuk digunakan sebagai PIX billing_details.tax_id."""
...
def run_pix_provider_attempt(
    ...
    _log(
        ...
        f"(PIX mungkin muncul setelah sinkronisasi pajak)",
    )
...
def generate_opll_pix_long_link(
    ...
    """Entri Eksternal: Ekstensi dengan tanda tangan yang sama seperti generate_opll_*_long_link lainnya."""
    ...
    raise RuntimeError("Ekstraksi PIX gagal: " + "; ".join(failures[-5:]))
