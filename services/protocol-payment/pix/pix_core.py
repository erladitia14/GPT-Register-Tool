...
def opll_build_chatgpt_session(access_token: str, proxy_url: str = "") -> requests.Session:
    ...
    if not token:
        raise RuntimeError("Akun saat ini tidak memiliki Access Token, harap daftar dan dapatkan informasi Session terlebih dahulu")
...
def opll_billing_for_country(country: str) -> dict:
    ...
    if country == "DE":
        ...
        raise RuntimeError(f"Wilayah data tagihan tidak didukung: {country}")
...
def opll_stripe_payload_diagnostics(payload, ctx: dict) -> str:
    ...
    return (
        f"submission_state={submission_state or 'tidak diketahui'}, submission_reason={submission_reason or 'tidak ada'}, "
        f"submission_code={submission_code or 'tidak ada'}, submission_message={submission_message or 'tidak ada'}, "
...
def opll_chatgpt_approve_with_retry(access_token: str, cs_id: str, checkout: dict, proxy_url: str = "") -> requests.Session:
    ...
    for _attempt in range(OPLL_APPROVE_BLOCKED_RETRY_MAX):
        try:
            chatgpt = opll_build_chatgpt_session(access_token, proxy_url)  # Setiap session baru (oai-device-id baru)
            opll_chatgpt_approve(chatgpt, cs_id, checkout)
            return chatgpt
        except OpllChatgptApproveBlocked as exc:
            # Blokir adalah kontrol risiko probabilistik, bisa dicoba ulang (referensi fufu.best); delay jitter lalu lanjut, jangan break
            last_error = str(exc)
            ...
            last_error = str(exc)
            last_was_blocked = False
            # 403/exception jaringan biasanya rate limit: backoff lebih lama, hindari pukulan beruntun yang memicu 403 lagi
            time.sleep(OPLL_APPROVE_BLOCKED_ERROR_BACKOFF + random.random())
    hint = "（Kontrol risiko probabilistik diblokir；dapat meningkatkan OPLL_APPROVE_BLOCKED_RETRY_MAX, atau gunakan proxy residensial bersih untuk meningkatkan tingkat kelulusan dasar）" if last_was_blocked else ""
    raise RuntimeError(f"ChatGPT approve berturut-turut {OPLL_APPROVE_BLOCKED_RETRY_MAX} kali tidak lolos{hint}: {last_error}")
