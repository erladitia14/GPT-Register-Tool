...
Variabel lingkungan yang umum digunakan:
  IDEAL_BANK=n26              # Mengikuti gpthel, default n26; bisa diubah ke ing dll
  IDEAL_CONFIRM_INLINE_PM=0   # Mengikuti alur gpthel: buat PM dulu, lalu confirm referensi PM
  IDEAL_UPDATE_TAX_REGION=1   # Proses iDEAL sinkronkan wilayah pajak ChatGPT/Stripe
  IDEAL_BOOTSTRAP_COUNTRY=NL  # Wilayah Checkout / inisialisasi Stripe pertama
  IDEAL_PROMOTION_COUNTRY=VN  # Wilayah checkout/update
  IDEAL_PROVIDER_COUNTRY=NL   # Wilayah refresh Stripe / pajak / PM / approve
  IDEAL_MAX_RETRY=900         # Opsional; default 30 putaran per batch, maks 30 batch
  IDEAL_PROVIDER_PER_CHECKOUT=30
  IDEAL_MAX_ACCOUNT_ATTEMPTS=0   # Jika diatur 0, jalankan sesuai kapasitas total pool proxy
  IDEAL_MAX_APPROVE_BLOCKED=900
  IDEAL_WORKERS=30            # Jumlah konkurensi proses utama; default 30 provider per batch
  IDEAL_WORKERS_MAX=30
  IDEAL_APPROVE_RETRY_MAX=10 # Ulangi approve dengan proxy provider saat ini
  IDEAL_APPROVE_STICKY=1     # Prioritas exit histori/saat ini, gagal ganti ke provider exit berikutnya
  IDEAL_FOLLOW_REDIRECT=1
  IDEAL_REQUIRE_ZERO=1        # Default paksa 0, tidak buat chain non-zero
  IDEAL_DUMP_LIMIT=6000       # Panjang respons penyimpanan dump
  IDEAL_PROXY_SKIP_FAILED=1   # Jika gagal dalam proses biasa, skip lunak selanjutnya
  IDEAL_PROXY_REMOVE_FAILED=1 # Proxy gagal jelas akan dihapus dari proxy_seeds.txt
  IDEAL_PROXY_DEFAULT_SCHEME=http # Skema default proxy polos; Mars SOCKS5 bisa atur socks5h
  IDEAL_PROXY_FAIL_COOLDOWN=180 # Waktu pending kegagalan proxy (detik), 0 berarti tetap skip sesuai logika lama
  IDEAL_PROXY_REMOVE_AFTER_FAILS=3 # Hapus setelah akumulasi 3 kali gagal kesehatan proxy yang di-reuse; hapus jika proxy biasa gagal 1 kali
  IDEAL_ZERO_CACHE=1          # Catat hasil observasi zero checkout untuk log dan troubleshooting
  IDEAL_ZERO_CACHE_SCHEDULING=0 # Atur eksplisit ke 1 untuk filter/prioritas penjadwalan berdasarkan hasil observasi zero
...
DEFAULT_TIMEOUT = 30
CHATGPT_TIMEOUT = 45
IDEAL_UNAVAILABLE_ERROR = "Metode pembayaran akun saat ini tidak mendukung iDEAL"
STRIPE_VERSION_FULL = (
...
def update_checkout_promotion(
    ...
    log(f"{IDEAL_PROMOTION_COUNTRY} checkout/update berhasil: promo={promo_id if 'promo_campaign' in body else 'off'}")
...
def update_ideal_checkout_taxes(
    ...
    log(f"{IDEAL_PROVIDER_COUNTRY} sinkronisasi checkout/taxes berhasil")
...
def resolve_confirm_payload_ideal(
    ...
    if not redirect_url and submission.get("state") == "requires_approval":
        ...
        log("Perlu approve: skenario iDEAL 0 nilai, utamakan proxy Provider yang berhasil sebelumnya/saat ini, ganti ke proxy Provider berikutnya jika gagal.")
...
def run_provider_flow(
    ...
    log(
        f"{IDEAL_PROMOTION_COUNTRY} checkout/update lalu refresh Stripe melalui {IDEAL_PROVIDER_COUNTRY}: "
    ...
    ctx, amount = inspect_init(
        init_payload, f"{IDEAL_PROMOTION_COUNTRY} setelah update {IDEAL_PROVIDER_COUNTRY}"
    )
    ...
    if amount == 0:
        log("Jumlah setelah Promosi adalah 0, lanjutkan dengan ekstraksi URL akhir untuk pindai/otorisasi menggunakan alur iDEAL 0 yuan")

    stripe = new_session(provider_proxy)
    stripe.headers.update({"User-Agent": random_user_agent(), "Accept-Language": payment_accept_language()})

    if env_bool("IDEAL_UPDATE_TAX_REGION", False):
        log(f"Sinkronkan checkout/taxes {IDEAL_PROVIDER_COUNTRY} dengan Stripe tax_region...")
        ...
        ctx, amount = inspect_init(init_payload, f"{IDEAL_PROVIDER_COUNTRY} sinkronisasi pajak")
        record_checkout_zero_result(checkout_proxy, checkout_country, amount)
        if amount != 0:
            raise RuntimeError(f"0 元优惠未生效，当前金额小单位={amount}，已停止生成非 0 元 iDEAL 链")
...
def run_once(
    ...
    log(f"Mulai ekstraksi iDEAL, percobaan ke {attempt}/{max_retry}")
    log(
        "Tes kombinasi: "
    ...
    try:
        ...
        log(f"Proxy ronde ini: checkout/kualifikasi={proxy_label(checkout_proxy)}；Stripe/iDEAL={proxy_label(provider_proxy)}")
...
def run_attempt(
    ...
    try:
        ...
        if is_ideal_unavailable_error(error):
            log(f"Percobaan {attempt}/{max_retry} checkout tidak menyediakan iDEAL, pertahankan proxy dan lanjutkan kombinasi selanjutnya", "[WARN] ")
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, error
    ...
    try:
        ...
        try:
            ...
            if is_checkout_not_active_error(error):
                ...
                log("Checkout saat ini tidak menyediakan iDEAL, ganti ke kombinasi checkout berikutnya", "[WARN] ")
                return attempt, "", error, False
            else:
...
def run_single_link_parallel_mode(
    ...
    if requested_workers > workers:
        log(f"konkurensi iDEAL dibatasi dari {requested_workers} menjadi {workers}", "[WARN] ")
    log(
        "Mulai proses ekstraksi rantai iDEAL:"
        f"checkout={checkout_country}/{checkout_currency}，PM={configured_pm_country}，locale={payment_browser_locale()}，"
        f"Checkout coba ulang={checkout_retry}, Provider coba ulang={provider_retry}, iDEAL total coba ulang={ideal_retry}, iDEAL konkurensi={workers}."
    )
...
def run_single_link_mode(
    ...
    if ideal_workers > 1:
        log(f"konkurensi rantai iDEAL tetap=1, abaikan IDEAL_WORKERS={ideal_workers}", "[WARN] ")
    ...
    log(
        "Mulai proses ekstraksi rantai iDEAL:"
        f"checkout={checkout_country}/{checkout_currency}，PM={configured_pm_country}，locale={payment_browser_locale()}，"
        f"Checkout coba ulang={checkout_retry}, Provider coba ulang={provider_retry}, iDEAL total coba ulang={ideal_retry}."
    )
    ...
    for attempt in range(1, ideal_retry + 1):
        ...
        try:
            ...
            if is_checkout_not_active_error(error):
                ...
                log("Checkout saat ini tidak menyediakan iDEAL, ganti ke ekstraksi berikutnya", "[WARN] ")
            else:
                record_failure_by_stage(error, checkout_proxy_used, provider_proxy, promotion_proxy)
                log(f"Provider gagal: {error[:220]}", "[WARN] ")
                if is_approve_failure_error(error) and "approve blocked" in error:
...
def main() -> int:
    ...
    if flow_mode != "single":
        log(f"IDEAL_FLOW_MODE={flow_mode} telah konvergen ke jalur single seed ketat", "[WARN] ")
