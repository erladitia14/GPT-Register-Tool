...
Penjelasan:
- TWINT di Stripe biasanya akan dialihkan ke halaman otorisasi pembayaran.
- Skrip ini akan mengekstrak redirect URL TWINT; URL tersebut adalah halaman pembayaran akhir jika berhasil.
- Secara default hanya membaca proxy_seeds.txt proyek saat ini.
...
Penggunaan:
  ...
  2) Cara berkas:
     Letakkan token di token.txt di direktori saat ini
     python twint_extract.py

Variabel lingkungan yang umum digunakan:
  TWINT_CONFIRM_INLINE_PM=0   # Mengikuti alur gpthel: buat PM dulu, lalu confirm referensi PM
  TWINT_UPDATE_TAX_REGION=1   # Proses TWINT sinkronkan wilayah pajak ChatGPT/Stripe
  TWINT_BOOTSTRAP_COUNTRY=CH  # Wilayah Checkout / inisialisasi Stripe pertama
  TWINT_PROMOTION_COUNTRY=VN  # Wilayah checkout/update
  TWINT_PROVIDER_COUNTRY=CH   # Wilayah refresh Stripe / pajak / PM / approve
  TWINT_MAX_RETRY=900         # Opsional; default 30 putaran per batch, maks 30 batch
  TWINT_PROVIDER_PER_CHECKOUT=30
  TWINT_MAX_ACCOUNT_ATTEMPTS=0   # Jika diatur 0, jalankan sesuai kapasitas total pool proxy
  TWINT_MAX_APPROVE_BLOCKED=900
  TWINT_WORKERS=30            # Jumlah konkurensi proses utama; default 30 provider per batch
  TWINT_WORKERS_MAX=30
  TWINT_APPROVE_RETRY_MAX=10 # Ulangi approve dengan proxy provider saat ini
  TWINT_APPROVE_STICKY=1     # Prioritas exit histori/saat ini, gagal ganti ke provider exit berikutnya
  TWINT_FOLLOW_REDIRECT=1
  TWINT_REQUIRE_ZERO=1        # Default paksa 0, tidak buat chain non-zero
  TWINT_DUMP_LIMIT=6000       # Panjang respons penyimpanan dump
  TWINT_PROXY_SKIP_FAILED=1   # Jika gagal dalam proses biasa, skip lunak selanjutnya
  TWINT_PROXY_REMOVE_FAILED=1 # Proxy gagal jelas akan dihapus dari proxy_seeds.txt
  TWINT_PROXY_DEFAULT_SCHEME=http # Skema default proxy polos; Mars SOCKS5 bisa atur socks5h
  TWINT_PROXY_FAIL_COOLDOWN=180 # Waktu pending kegagalan proxy (detik), 0 berarti tetap skip sesuai logika lama
  TWINT_PROXY_REMOVE_AFTER_FAILS=3 # Hapus setelah akumulasi 3 kali gagal kesehatan proxy yang di-reuse; hapus jika proxy biasa gagal 1 kali
  TWINT_ZERO_CACHE=1          # Catat hasil observasi zero checkout untuk log dan troubleshooting
  TWINT_ZERO_CACHE_SCHEDULING=0 # Atur eksplisit ke 1 untuk filter/prioritas penjadwalan berdasarkan hasil observasi zero
  PP_PROMO_MODE=campaign      # Default langsung jalankan promo_campaign, hindari fallback coupon yang makan waktu
  PP_TRIAL_DAYS=30            # Hanya digunakan jika PP_PROMO_MODE=trial/free_trial
"""
...
def print_result_url(url: str) -> None:
    print(ProtocolResult(
        ...
        status="completed",
        url=url,
        link_type="twint_protocol",
    ).to_json())

DEFAULT_TIMEOUT = 30
CHATGPT_TIMEOUT = 45
TWINT_UNAVAILABLE_ERROR = "Metode pembayaran akun saat ini tidak mendukung TWINT"
STRIPE_VERSION_FULL = (
...
def configured_country(name: str, default: str) -> str:
    ...
    if not re.fullmatch(r"[A-Z]{2}", value):
        raise RuntimeError(f"{name} harus kode negara dua huruf")
    return value
...
def proxy_for_country(proxy: str, country: str) -> str:
    ...
    if not replacements:
        raise RuntimeError(
            f"Proxy tidak mengandung pemilih country/region yang bisa ditimpa: {proxy_label(proxy)}"
        )
...
def load_proxy_seeds() -> list[str]:
    ...
    log(
        "Kebijakan proxy ketat: ambil satu seed per ronde, turunkan "
    ...
    log(f"Proxy depan lokal: {proxy_short(pre_proxy_url())}")
    return proxy_seeds
...
def update_checkout_promotion(
    ...
    if resp.status_code >= 400:
        ...
        raise RuntimeError(f"checkout/update gagal HTTP {resp.status_code}: {resp.text[:500]}")
    try:
    ...
    log(f"{TWINT_PROMOTION_COUNTRY} checkout/update berhasil: promo={promo_id if 'promo_campaign' in body else 'off'}")
...
def update_twint_checkout_taxes(
    ...
    if resp.status_code >= 400:
        if is_checkout_not_active_error(resp.text):
            raise RuntimeError("checkout_not_active_session")
        raise RuntimeError(f"checkout/taxes gagal HTTP {resp.status_code}: {resp.text[:500]}")
    log(f"{TWINT_PROVIDER_COUNTRY} sinkronisasi checkout/taxes berhasil")
...
def stripe_create_twint_pm(stripe: requests.Session, cs_id: str, stripe_pk: str, billing: dict[str, str], ctx: dict[str, Any]) -> str:
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"Gagal membuat TWINT PM HTTP {resp.status_code}: {resp.text[:500]}")
    pm_id = str((resp.json() or {}).get("id") or "")
    if not pm_id.startswith("pm_"):
        raise RuntimeError(f"Tanggapan pembuatan TWINT PM tidak normal: {resp.text[:300]}")
    return pm_id
...
def stripe_confirm_twint(
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"TWINT confirm gagal HTTP {resp.status_code}: {resp.text[:500]}")
    return resp.json() or {}
...
def raise_if_setup_intent_blocked(payload: Any, context: str, current_pm_id: str = "") -> None:
    ...
    if "generic_decline" in last_error.lower():
        raise RuntimeError(f"Stripe menolak karena kontrol risiko (generic_decline): {context} Pembuatan SetupIntent gagal, redirect_url tidak dibuat")
...
def stripe_intent_redirect_url(
    ...
    if redirect_url:
        ...
        log(f"Baca Stripe intent dapatkan redirect_url: {redirect_url[:180]}")
    return redirect_url
...
def log_approve_failure(error: str) -> bool:
    ...
    if "ChatGPT approve 未通过: blocked" in error:
        log("Approve mengembalikan blocked, tangani sebagai kontrol risiko akun/checkout, jangan catat kegagalan proxy", "[WARN] ")
        return True
    return False
...
def approve_with_retry(
    ...
    if sticky and parallel > 1:
        log("Gagal approve ganti proxy, jalankan secara berurutan sesuai urutan kandidat")
        parallel = 1
    if sticky:
...
def poll_payment_page(
    ...
    log(f"poll selesai, tidak menemukan redirect/QR TWINT asli: {last_error}", "[WARN] ")
...
def resolve_confirm_payload_twint(
    ...
    if redirect_url:
        log(f"confirm berhasil mengekstrak URL pembayaran akhir: {redirect_url[:180]}")
    if qr_urls:
    ...
    if not redirect_url and submission.get("state") == "requires_approval":
        ...
        log("Perlu approve: skenario TWINT 0 nilai, utamakan proxy Provider yang berhasil sebelumnya/saat ini, ganti ke proxy Provider berikutnya jika gagal.")
        ...
        log("confirm tidak mengembalikan redirect TWINT asli, lanjutkan poll payment_pages untuk konfirmasi akhir", "[WARN] ")
        redirect_url, poll_qr = poll_payment_page(stripe, checkout, stripe_pk, ctx, current_pm_id=pm_id)
        qr_urls.extend(poll_qr)
...
def run_provider_flow(
    ...
    def inspect_init(payload: dict[str, Any], stage: str) -> tuple[dict[str, Any], int]:
        ...
        log(f"{stage} Stripe init berhasil, jumlah={checkout['currency']} {amount_major:.2f}")
    ...
    log(
        f"{TWINT_PROMOTION_COUNTRY} checkout/update lalu refresh Stripe melalui {TWINT_PROVIDER_COUNTRY}: "
    ...
    ctx, amount = inspect_init(
        init_payload, f"{TWINT_PROMOTION_COUNTRY} setelah update {TWINT_PROVIDER_COUNTRY}"
    )
    ...
    if amount == 0:
        log("Jumlah setelah Promosi adalah 0, lanjutkan dengan ekstraksi URL pembayaran akhir menggunakan alur TWINT 0 yuan")

    stripe = new_session(provider_proxy)
    stripe.headers.update({"User-Agent": random_user_agent(), "Accept-Language": payment_accept_language()})

    if env_bool("TWINT_UPDATE_TAX_REGION", False):
        log(f"Sinkronkan checkout/taxes {TWINT_PROVIDER_COUNTRY} dengan Stripe tax_region...")
        ...
        ctx, amount = inspect_init(init_payload, f"{TWINT_PROVIDER_COUNTRY} sinkronisasi pajak")
        record_checkout_zero_result(checkout_proxy, checkout_country, amount)
        if amount != 0:
            raise RuntimeError(f"0 元优惠未生效，当前金额小单位={amount}，已停止生成非 0 元 TWINT 链")
    ...
    if env_bool("TWINT_CONFIRM_INLINE_PM", False):
        log(
            f"Data inline TWINT confirm: {billing['name']} / "
            f"{billing['line1']} / {billing['city']} {billing['postal_code']}"
        )
    else:
        log(f"Buat PM (TWINT): {billing['country']} {billing['name']} / {billing['city']}")
        pm_id = stripe_create_twint_pm(stripe, checkout["cs_id"], stripe_pk, billing, ctx)
        log(f"PM berhasil dibuat: {pm_id}")
        log(
            f"Data Swiss sudah diisi ke PM billing_details: {billing['name']} / "
            f"{billing['line1']} / {billing['city']} {billing['postal_code']}"
        )

    if env_bool("TWINT_UPDATE_CUSTOMER_DATA", False):
        log(
            f"Kirim pengisian data Swiss: {billing['name']} / {billing['line1']} / "
    ...
    try:
        ...
        if not should_retry_second_confirm_after_approve(exc):
            raise
        log(f"Tidak mendapat redirect setelah approve/confirm, segarkan Stripe init lalu confirm kedua kali: {str(exc)[:180]}", "[WARN] ")
        init_payload = stripe_init(checkout["cs_id"], stripe_pk, provider_proxy)
        hosted_url = str(init_payload.get("stripe_hosted_url") or hosted_url or "")
        ctx = build_ctx(init_payload, checkout)
        confirm_payload = stripe_confirm_twint(stripe, checkout["cs_id"], pm_id, stripe_pk, init_payload, ctx, checkout, hosted_url, billing)
        log("Stripe konfirmasi kedua berhasil, parsing redirect...")
...
def run_once(
    ...
    log(f"Mulai ekstraksi TWINT, percobaan ke {attempt}/{max_retry}")
    log(
        "Tes kombinasi: "
    ...
    try:
        ...
        log(f"Proxy ronde ini: checkout/kualifikasi={proxy_label(checkout_proxy)}；Stripe/TWINT={proxy_label(provider_proxy)}")
...
def run_attempt(
    ...
    try:
        ...
        if is_twint_unavailable_error(error):
            log(f"Percobaan {attempt}/{max_retry} checkout tidak menyediakan TWINT, pertahankan proxy dan lanjutkan kombinasi selanjutnya", "[WARN] ")
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, error
...
def run_single_link_attempt(
    ...
    try:
        ...
        log(f"Mulai percobaan ke-{attempt}/{twint_retry} untuk ambil tautan")
        log(
            f"Langkah 1: Buat checkout ChatGPT... tagihan checkout={checkout_country}/{checkout_currency},"
            f"Percobaan ke-{attempt}/{twint_retry}, setiap kali ambil maksimal {checkout_retry} node secara acak"
        )
        ...
        try:
            ...
            if is_checkout_not_active_error(error):
                ...
                log("Checkout saat ini tidak menyediakan TWINT, ganti ke kombinasi checkout berikutnya", "[WARN] ")
                return attempt, "", error, False
            else:
                record_failure_by_stage(error, checkout_proxy_used, provider_proxy, promotion_proxy)
                log(f"Provider gagal: {error[:220]}", "[WARN] ")
                if is_approve_failure_error(error) and "approve blocked" in error:
                    approve_blocked = True

        _log_context.prefix = f"[TWINT {attempt}/{twint_retry}] "
        log(f"Percobaan ke-{attempt}/{twint_retry} untuk ambil tautan selesai, tidak mendapatkan URL akhir", "[WARN] ")
...
def run_single_link_parallel_mode(
    ...
    if requested_workers > workers:
        log(f"Konkurensi TWINT dibatasi dari {requested_workers} menjadi {workers}", "[WARN] ")
    log(
        "Mulai proses ekstraksi rantai TWINT:"
        f"checkout={checkout_country}/{checkout_currency}，PM={configured_pm_country}，locale={payment_browser_locale()}，"
        f"Checkout coba ulang={checkout_retry}, Provider coba ulang={provider_retry}, TWINT total coba ulang={twint_retry}, TWINT konkurensi={workers}."
    )
    ...
    try:
        ...
        for future in as_completed(futures):
            try:
                ...
                error = str(exc)
                approve_blocked = False
                log(f"Percobaan {attempt}/{twint_retry} ekstraksi tautan pengecualian: {error[:300]}", "[WARN] ")
            if redirect_url:
                stop_event.set()
                for pending in futures:
                    pending.cancel()
...
def run_single_link_mode(
    ...
    if twint_workers > 1:
        log(f"Ekstraksi tautan TWINT memiliki konkurensi tetap=1, abaikan TWINT_WORKERS={twint_workers}", "[WARN] ")
    ...
    log(
        "Mulai proses ekstraksi rantai TWINT:"
        f"checkout={checkout_country}/{checkout_currency}，PM={configured_pm_country}，locale={payment_browser_locale()}，"
        f"Checkout coba ulang={checkout_retry}, Provider coba ulang={provider_retry}, TWINT total coba ulang={twint_retry}."
    )
    ...
    for attempt in range(1, twint_retry + 1):
        ...
        for checkout_index, proxy_seed in enumerate(checkout_candidates, start=1):
            ...
            try:
                ...
                log(
                    ...
                    f"Percobaan ini telah mencoba Seed={len(attempted_seed_keys)}"
                )
                zero_status, zero_amount, _zero_checked_at = checkout_zero_cache_status(checkout_proxy, checkout_country)
                if zero_status == "ok":
                    log(f"cache hit kualifikasi checkout Rp 0: amount={zero_amount}")
        ...
        try:
            ...
            if is_checkout_not_active_error(error):
                ...
                log("Checkout saat ini tidak menyediakan TWINT, ganti ke ekstraksi berikutnya", "[WARN] ")
            else:
                record_failure_by_stage(error, checkout_proxy_used, provider_proxy, promotion_proxy)
                log(f"Provider gagal: {error[:220]}", "[WARN] ")
                if is_approve_failure_error(error) and "approve blocked" in error:
...
def main() -> int:
    ...
    if flow_mode != "single":
        log(f"TWINT_FLOW_MODE={flow_mode} telah menyatu menjadi jalur strict single seed", "[WARN] ")
