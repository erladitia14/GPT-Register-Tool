"""
	Skrip pengiriman antarmuka BLIK / ekstraksi URL halaman pembayaran iDEAL.

Penjelasan:
- Mode BLIK mengirimkan 6 digit BLIK Code dan data tagihan Polandia melalui antarmuka Stripe, tidak membuka halaman pembayaran.
- iDEAL di Stripe biasanya adalah redirect otorisasi bank, belum tentu ada halaman instruksi QR.
- Skrip ini akan mengekstrak redirect URL iDEAL; URL tersebut adalah antarmuka pindai/otorisasi akhir jika berhasil.
- Secara default hanya membaca proxy_seeds.txt proyek saat ini; setiap Seed akan diturunkan menjadi PL checkout, Stripe, dan exit approve.

Penggunaan:
  1) Metode variabel lingkungan:
     PP_TOKEN="..." python blik_qr_extract.py

  2) Mode file:
     letakkan token di direktori saat ini token.txt
     python blik_qr_extract.py

Variabel lingkungan umum:
  IDEAL_PAYMENT_METHOD=blik   # blik atau ideal; default blik
  IDEAL_BLIK_CODE=            # BLIK wajib: kode bank 6 digit; jika kosong, tugas berhenti
  IDEAL_BANK=ing              # iDEAL opsional: tentukan bank; jika kosong, ke halaman pilihan bank
  IDEAL_CONFIRM_INLINE_PM=0   # default mengikuti alur gpthel: buat PM dulu, lalu confirm dengan PM
  IDEAL_UPDATE_TAX_REGION=0   # default tidak kirim tambahan tax_region
  IDEAL_CHECKOUT_SNAPSHOT=0   # default tidak kirim tambahan checkout snapshot
  IDEAL_CHECKOUT_COUNTRY=PL   # BLIK default PL; iDEAL bisa set NL
  IDEAL_MAX_RETRY=900         # opsional; jika tidak di-set: default 30 putaran per batch, maksimal 30 batch
  IDEAL_PROVIDER_PER_CHECKOUT=30
  IDEAL_MAX_ACCOUNT_ATTEMPTS=0   # jika di-set eksplisit ke 0, jalankan sesuai kapasitas total kumpulan proxy
  IDEAL_MAX_APPROVE_BLOCKED=900
  IDEAL_WORKERS=30            # jumlah konkurensi alur utama; default 30 provider berjalan bersamaan per batch
  IDEAL_WORKERS_MAX=30
  IDEAL_APPROVE_RETRY_MAX=10 # jumlah maksimal percobaan ulang tahap approve
  IDEAL_APPROVE_STICKY=0     # approve default berganti gateway melalui kumpulan proxy
  IDEAL_FOLLOW_REDIRECT=1
  IDEAL_REQUIRE_ZERO=1        # default paksa 0 rupiah, tidak menghasilkan rantai bukan 0 rupiah
  IDEAL_DUMP_LIMIT=6000       # panjang penyimpanan tanggapan tangkap paket
  IDEAL_PROXY_SKIP_FAILED=1   # gagal alur biasa, lewati lembut lain kali
  IDEAL_PROXY_REMOVE_FAILED=1 # proxy yang gagal secara eksplisit akan dihapus dari file proxy asli
  ...
  IDEAL_PROXY_DEFAULT_SCHEME=http # skema default proxy mentah; Mars SOCKS5 bisa set socks5h
  IDEAL_PROXY_GEO_USE_PRE_PROXY=0 # 国家检测默认不走本机前置，避免检测到 Clash 出口国家
  IDEAL_PROXY_TARGET_CHECK=1  # pra-penyaringan/saat runtime pastikan proxy dapat akses chatgpt.com/api.stripe.com
  IDEAL_PROXY_TARGET_USE_PRE_PROXY=1 # deteksi situs target default gunakan pre-proxy lokal, cocokkan rantai akses formal
  IDEAL_PROXY_FAIL_COOLDOWN=180 # detik pendingin proxy gagal, 0 artinya selalu lewati seperti logika lama
  IDEAL_PROXY_REMOVE_AFTER_FAILS=3 # hapus proxy yang sudah digunakan ulang setelah akumulasi 3 kegagalan tipe kesehatan; hapus proxy biasa setelah 1 kegagalan
  IDEAL_ZERO_CACHE=1          # cache hasil kelayakan checkout 0 rupiah, untuk penjadwalan prioritas selanjutnya
  IDEAL_PROXY_SEED_FILE=proxy_seeds.txt
  PP_PROMO_MODE=campaign      # default langsung jalankan promo_campaign, hindari kupon lalu fallback menambah waktu
  PP_TRIAL_DAYS=30            # hanya digunakan saat PP_PROMO_MODE=trial/free_trial"""
...
def print_result_url(url: str) -> None:
    if payment_method_type() == "blik" and os.environ.get("IDEAL_BLIK_CODE", "").strip():
        print("BLIK kirim otomatis selesai")
        # Sentinel penyelesaian terstruktur: mode pengiriman otomatis BLIK mengirim 6 digit ke Stripe untuk menyelesaikan pembayaran, hasilnya
        # status "Pembayaran Selesai" bukan URL yang dapat dibagikan. payment_link_manager bergantung pada baris ini untuk menentukan keberhasilan,
        # menghindari pengambilan/pengidentifikasian URL yang salah dari log yang terpotong.
        print("BLIK_RESULT:" + ProtocolResult(
            ...
            message="BLIK kirim otomatis selesai",
...
def validate_blik_code_before_start() -> bool:
    ...
    if not code:
        log("BLIK Code kosong, tugas berakhir; harap isi BLIK Code 6 digit terlebih dahulu", "[ERROR] ")
        return False
    if not re.fullmatch(r"\d{6}", code):
        log("BLIK Code harus 6 digit angka, tugas berakhir", "[ERROR] ")
        return False
    return True
...
def proxy_for_country(proxy: str, country: str) -> str:
    ...
    if not proxy:
        raise RuntimeError("Proxy kosong, tidak bisa turunkan link region")
    ...
    if not replacements:
        raise RuntimeError(f"Proxy tidak mengandung pemilih country/region yang bisa ditimpa: {proxy_label(proxy)}")
...
def blik_proxy_chain(proxy_seed: str) -> tuple[str, str]:
    ...
    if not chain_key or any(proxy_chain_key(proxy) != chain_key for proxy in (checkout_proxy, provider_proxy)):
        raise RuntimeError("Penggantian wilayah proxy mengubah sticky seed, pencampuran proxy chain ditolak")
...
def prune_proxy_state(checkout_proxies: list[str], provider_proxies: list[str]) -> None:
    ...
    if removed_counts:
        ...
        log(f"Pembersihan status proxy selesai: {summary}")
...
def prune_proxy_seed_state(proxy_seeds: list[str]) -> None:
    with _proxy_state_lock:
        ...
        if stale_keys:
            save_proxy_state()
    if stale_keys:
        log(f"Pembersihan status proxy Seed selesai: {len(stale_keys)}")
...
def ensure_proxy_country(group: str, proxy: str) -> None:
    ...
    log(
        f"{group} deteksi keluar: {proxy_label(proxy)} country={country or 'TIDAK DIKETAHUI'} "
...
def ensure_proxy_targets(group: str, proxy: str) -> None:
    ...
    log(f"{group} deteksi target: {proxy_label(proxy)} reachable={ok} source={reason}")
    if not ok:
        raise RuntimeError(f"{group} proxy target tidak dapat dijangkau: {reason}")
...
def precheck_proxy_group(group: str, proxies: list[str]) -> list[str]:
    ...
    if requested_workers > workers:
        log(f"{group} konkurensi pra-skrining proxy dibatasi dari {requested_workers} menjadi {workers}", "[WARN] ")
    log(
        f"{group} pra-skrining proxy dimulai: total={total}, "
        ...
        for index, future in enumerate(as_completed(futures), start=1):
            ...
            if country_ok and target_ok:
                ...
                if not country_ok and not country:
                    ...
                    reason = f"Target situs prasaringan tidak dapat dijangkau: {target_reason}"
                    target_failures.append((proxy, reason))
                    record_proxy_health_failure(group, proxy, reason)
            if index % 50 == 0 or index == total:
                log(f"{group} progres pra-skrining proxy: {index}/{total}, kept={len(kept_set)}, failed={failed}, unknown={unknown}")
    ...
    if not kept_set:
        ...
        if remaining:
            log(f"{group} pra-skrining proxy tidak ada hasil yang jelas tersedia, hanya menyisakan {len(remaining)} proxy dengan keluar tidak diketahui untuk dijalankan", "[WARN] ")
        return remaining
    kept = [proxy for proxy in proxies if proxy in kept_set]
    log(f"{group} pra-skrining proxy selesai: kept={len(kept)}/{total}, removed={total - len(kept)}, unknown={unknown}")
    return kept
...
def order_proxy_group(group: str, proxies: list[str]) -> list[str]:
    ...
    if not kept and proxies:
        log(f"status proxy {group} setelah filter kosong, semua dilewati", "[WARN] ")
    ...
    if cooldown_skipped or success_seen or zero_seen or zero_skipped:
        log(
            f"status proxy {group}: prioritas sukses={success_seen}, tembus 0-Rupiah={zero_seen}, "
            f"Lewati cooling={cooldown_skipped}, lewati kegagalan 0 dolar={zero_skipped}"
        )
    return ordered
...
def pre_proxy_url() -> str:
    """Proxy lokal: proxy lokal -> proxy file -> target site."""
...
def unique_proxy_seeds(proxy_seeds: list[str]) -> list[str]:
    ...
    if duplicates:
        log(f"penyaringan duplikat seed proxy: mengabaikan {duplicates} sticky session yang sama", "[WARN] ")
    return unique
...
def load_proxy_seeds() -> list[str]:
    if env_bool("IDEAL_USE_LOCAL_PROXY_ONLY", False):
        ...
        if not local_proxy:
            raise RuntimeError("Mode koneksi langsung proxy lokal membutuhkan pengaturan IDEAL_PRE_PROXY")
        log("Mode koneksi langsung proxy lokal: satu ekspor untuk seluruh alur BLIK")
        return [local_proxy]

    path = proxy_seed_file()
    if not path.is_file():
        raise RuntimeError("File Proxy Seed tidak ada")
    proxy_seeds = unique_proxy_seeds(load_proxy_file(path))
    if not proxy_seeds:
        raise RuntimeError("Proxy Seed kosong")
    prune_proxy_seed_state(proxy_seeds)
    proxy_seeds = order_proxy_group("seed", proxy_seeds)
    if not proxy_seeds:
        raise RuntimeError("seed proxy sudah semuanya dalam pendingin kegagalan")
    log(f"Memuat {len(proxy_seeds)} Proxy Seed")
    log("Kebijakan proxy ketat: Ambil satu seed per putaran, turunkan PL Checkout → PL Stripe/BLIK → PL approve")
    log(f"Proxy default protokol: {default_proxy_scheme()}://")
    log(f"Proxy depan lokal: {proxy_label(pre_proxy_url())}")
    return proxy_seeds
...
def is_proxy_health_failure(reason: str) -> bool:
    ...
    return any(
        marker in text
        for marker in (
            "Target site tidak dapat dijangkau",
            "proxy-server",
            "connection reset",
            "recv failure",
            "timed out",
...
def load_proxy_groups() -> tuple[list[str], list[str]]:
    if env_bool("IDEAL_USE_LOCAL_PROXY_ONLY", False):
        ...
        if not local_proxy:
            raise RuntimeError("Mode koneksi langsung proxy lokal membutuhkan pengaturan IDEAL_PRE_PROXY")
        log(f"Mode koneksi langsung proxy lokal: checkout/provider menggunakan {proxy_label(local_proxy)}")
    ...
    if not checkout_proxies:
        raise RuntimeError("Proxy Checkout kosong: memerlukan checkout.json")
    if not provider_proxies:
        raise RuntimeError("Proxy Provider kosong: provider.json diperlukan pada tahap persetujuan (approve)")
    ...
    if not checkout_proxies:
        raise RuntimeError("Semua proxy Checkout telah difilter dengan status gagal")
    if not provider_proxies:
        raise RuntimeError("Semua Provider proxy telah disaring berdasarkan status gagal")
    log(f"Memuat {len(checkout_proxies)} agen Checkout: {checkout_file}")
    log(f"Memuat {len(provider_proxies)} agen Provider: {provider_file}")
    log(f"Kebijakan proxy: checkout/kelayakan 0/approve gunakan Checkout; Stripe/{payment_method_label()} gunakan Provider")
    log(f"Proxy default protokol: {default_proxy_scheme()}://")
    log(f"Proxy depan lokal: {proxy_label(pre_proxy_url())}")
    log(
        "Kebijakan pra-deteksi proxy: "
        f"Deteksi negara={'on' if env_bool('IDEAL_PROXY_GEO_USE_PRE_PROXY', False) else 'off'},"
        f"Deteksi target={'on' if env_bool('IDEAL_PROXY_TARGET_USE_PRE_PROXY', True) else 'off'}"
    )
    ...
    log(f"Proxy Checkout {len(checkout_proxies)} setelah prasaringan")
    log(f"Proxy Provider {len(provider_proxies)} setelah prasaringan")
...
def new_session(proxy: str = "", use_pre_proxy: bool = True) -> Any:
    ...
    if CurlCffiSession is not None:
        ...
        if pre_proxy:
            if CurlOpt is None:
                raise RuntimeError("Proxy lokal membutuhkan dukungan curl_cffi")
            kwargs["curl_options"] = {CurlOpt.PRE_PROXY: pre_proxy}
        session = CurlCffiSession(**kwargs)
    else:
        if pre_proxy:
            raise RuntimeError("Proxy lokal membutuhkan curl_cffi: python3 -m pip install curl_cffi")
...
def load_token() -> tuple[str, str]:
    for env_name in ("PP_TOKEN", "IDEAL_TOKEN"):
        ...
        if value:
            log(f"Gunakan variabel lingkungan {env_name}")
            ...
            if env_session or session_token:
                log("cookie sessionToken telah dimuat")
    ...
    token = input("Silakan masukkan access_token: ").strip()
...
def create_checkout(chatgpt: requests.Session, country: str) -> dict[str, str]:
    ...
    if promo_mode in ("trial", "free_trial"):
        ...
        log(f"PP_PROMO_MODE tidak diketahui={promo_mode!r}, diabaikan", "[WARN] ")
    ...
    if resp.status_code >= 400:
        if is_user_already_paid_error(resp.text):
            raise RuntimeError("Pengguna telah membayar: User is already paid")
    ...
    if (
        ...
        log("respons kupon tidak menampilkan promo, coba ulang dengan string promo_campaign", "[PROMO] ")
        ...
        if resp.status_code >= 400:
            if is_user_already_paid_error(resp.text):
                raise RuntimeError("Pengguna telah membayar: User is already paid")
            raise RuntimeError(f"promo_campaign checkout upaya ulang gagal HTTP {resp.status_code}: {resp.text[:500]}")
        data = resp.json() or {}
        log(f"promo_campaign setelah coba ulang promo={checkout_response_has_promo(data)}", "[PROMO] ")

    cs_id = data.get("checkout_session_id") or data.get("session_id") or data.get("id")
    if not cs_id or not str(cs_id).startswith("cs_"):
        raise RuntimeError(f"respons checkout kehilangan cs_id: {str(data)[:500]}")
    ...
    log(
        f"Checkout berhasil dibuat: {cs_id} / {country} / {currency_for_country(country)} / "
...
def stripe_init(cs_id: str, stripe_pk: str, proxy: str) -> dict[str, Any]:
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"Stripe init gagal HTTP {resp.status_code}: {resp.text[:500]}")
    payload = resp.json() or {}
...
def stripe_update_customer_data(
    ...
    try:
        ...
        if resp.status_code < 400:
            log(
                f"Data pelanggan Belanda telah dikirim: {billing['name']} / {billing['city']} / {billing['postal_code']}"
            )
            return True
        if is_checkout_not_active_error(resp.text):
            raise RuntimeError("checkout_not_active_session")
        log(f"Pengiriman data pelanggan Belanda gagal HTTP {resp.status_code}: {resp.text[:180]}", "[WARN] ")
    except Exception as exc:
        if is_checkout_not_active_error(exc):
            raise
        log(f"Pengiriman data pelanggan Belanda gagal: {exc}", "[WARN] ")
    return False
...
def stripe_update_tax_region(
    ...
    try:
        ...
        if resp.status_code < 400:
            log(f"tax_region sudah disubmit: {billing['country']} / {billing['city']} {billing['postal_code']}")
            return True
        if is_checkout_not_active_error(resp.text):
            raise RuntimeError("checkout_not_active_session")
        log(f"submisi tax_region gagal HTTP {resp.status_code}: {resp.text[:180]}", "[WARN] ")
    except Exception as exc:
        if is_checkout_not_active_error(exc):
            raise
        log(f"pengecualian submisi tax_region: {exc}", "[WARN] ")
    return False
...
def checkout_snapshot(chatgpt: requests.Session, checkout: dict[str, str], billing: dict[str, str]) -> None:
    ...
    try:
        ...
        if resp.status_code >= 400:
            if is_checkout_not_active_error(resp.text):
                raise RuntimeError("checkout_not_active_session")
            log(f"snapshot checkout gagal HTTP {resp.status_code}: {resp.text[:180]}", "[WARN] ")
        else:
            log("snapshot checkout sudah disubmit")
    except Exception as exc:
        if is_checkout_not_active_error(exc):
            raise
        log(f"pengecualian snapshot checkout: {exc}", "[WARN] ")
...
def stripe_create_ideal_pm(stripe: requests.Session, cs_id: str, stripe_pk: str, billing: dict[str, str], ctx: dict[str, Any]) -> str:
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"Gagal buat PM iDEAL HTTP {resp.status_code}: {resp.text[:500]}")
    pm_id = str((resp.json() or {}).get("id") or "")
    if not pm_id.startswith("pm_"):
        raise RuntimeError(f"Respon pembuatan PM iDEAL tidak normal: {resp.text[:300]}")
    return pm_id
...
def stripe_confirm_ideal(
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"iDEAL konfirmasi gagal HTTP {resp.status_code}: {resp.text[:500]}")
    return resp.json() or {}
...
def stripe_confirm_blik(
    ...
    code = os.environ.get("IDEAL_BLIK_CODE", "").strip()
    if not re.fullmatch(r"\d{6}", code):
        raise RuntimeError("IDEAL_BLIK_CODE harus 6 digit angka")
    body: dict[str, Any] = {
        "eid": "NA",
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"BLIK confirm gagal HTTP {resp.status_code}: {resp.text[:500]}")
    return resp.json() or {}
...
def raise_if_setup_intent_blocked(payload: Any, context: str) -> None:
    ...
    if "generic_decline" in last_error.lower():
        raise RuntimeError(f"Penolakan pengendalian risiko Stripe (generic_decline): {context} Pembuatan SetupIntent gagal; last_setup_error={last_error}")
...
def log_payment_page_summary(stage: str, payload: Any) -> None:
    ...
    log(f"{stage} mengembalikan ringkasan: {compact}")
...
def warmup_approve_context(chatgpt: requests.Session, checkout_page_url: str) -> None:
    try:
        ...
        log(f"pengecualian permintaan sentinel setuju: {exc}", "[WARN] ")
...
def chatgpt_approve(chatgpt: requests.Session, checkout: dict[str, str]) -> None:
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"ChatGPT approve gagal HTTP {resp.status_code}: {resp.text[:300]}")
    result = ""
    try:
        ...
        pass
    if result != "approved":
        raise RuntimeError(f"ChatGPT approve tidak lolos: {result or resp.text[:200]}")
...
def approve_attempt(
    ...
    log(f"setuju upaya ke {index}/{attempt_count} / proxy={proxy_label(proxy)}")
...
def log_approve_failure(error: str) -> bool:
    log(f"setuju gagal: {error[:180]}", "[WARN] ")
    if "ChatGPT approve 未通过: blocked" in error:
        log("approve mengembalikan blocked, saat ini checkout/approve grup gagal, coba berikutnya", "[WARN] ")
        return True
    return False
...
def approve_with_retry(
    ...
    last_error = ""
    if max_retry <= 0:
        raise RuntimeError("jumlah upaya setuju harus lebih dari 0")
    proxies = [proxy for proxy in dict.fromkeys(proxies) if proxy]
    if not proxies:
        raise RuntimeError("agen setuju kosong")
    ...
    log(f"strategi agen setuju: {'sticky' if sticky else 'rotate'}")
    if parallel > 1:
        workers = min(parallel, attempt_count)
        log(f"setuju konkurensi: pekerja={workers}, upaya={attempt_count}")
        blocked_count = 0
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(
        ...
        raise RuntimeError(f"approve coba ulang gagal: {last_error}")
...
def poll_payment_page(stripe: requests.Session, checkout: dict[str, str], stripe_pk: str, ctx: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    while time.time() < deadline:
        ...
        if summary_text and summary_text != last_summary:
            ...
            log(f"polling mengembalikan ringkasan: {summary_text}")
        ...
        if submission.get("state") == "requires_approval":
            last_error = "payment_pages masih requires_approval"
            time.sleep(1)
            continue
    ...
    log(f"poll selesai, tidak ditemukan redirect/QR iDEAL asli: {last_error}", "[WARN] ")
    return "", []
...
def poll_blik_status(
    ...
    while time.time() < deadline:
        ...
        if summary_text and summary_text != last_summary:
            last_summary = summary_text
            log(f"BLIK poll mengembalikan ringkasan: {summary_text}")
        ...
        if intent_status in {"requires_payment_method", "canceled"}:
            raise RuntimeError(f"BLIK pembayaran gagal: intent_status={intent_status}")
        if next_action_type == "blik_authorize":
            last_state = "Menunggu konfirmasi mobile banking"
        elif submission.get("state") == "requires_approval":
            last_state = "payment_pages masih requires_approval"
        else:
        ...
        time.sleep(1)
    raise RuntimeError(f"BLIK waktu konfirmasi status habis: {last_state}")
...
def resolve_external_redirect(stripe: requests.Session, start_url: str) -> str:
    ...
    for hop in range(1, 6):
        ...
        try:
            ...
            log(f"Pengecualian saat mengikuti redirect: {exc}", "[WARN] ")
            return current
        location = resp.headers.get("location") or resp.headers.get("Location") or ""
        if not location:
            return current
...
def approve_proxy_candidates(checkout_proxy: str, provider_proxy: str, approve_pool: list[str]) -> list[str]:
    ...
    if approve_preferences:
        log(f"Hit proxy sukses approve diprioritaskan: {proxy_label(approve_preferences[0])}")
...
def run_provider_flow(
    ...
    if not checkout.get("processor_entity"):
        ...
        if processor_entity:
            ...
            log(f"menyimpulkan processor_entity={processor_entity} dari Stripe init")
    ...
    log(f"Stripe init berhasil, jumlah={checkout['currency']} {amount_major:.2f}")
    ...
    if isinstance(payment_method_types, list):
        ...
        log(f"Metode pembayaran Stripe tersedia: {methods}")
    ...
    if int(amount or 0) == 0:
        action = "Kirim antarmuka BLIK" if expected_method == "blik" else "Ekstrak URL halaman pembayaran iDEAL"
        log(f"Jumlah saat ini 0, lanjutkan dengan alur promo 0 {action}")
    if stop_event and stop_event.is_set():
        raise RuntimeError("Tugas telah dihentikan, lewati putaran ini")

    if expected_method == "blik":
        stripe = new_session(provider_proxy)
        stripe.headers.update({"User-Agent": random_user_agent(), "Accept-Language": payment_accept_language()})
        log(
            f"BLIK antarmuka confirm: tagihan={billing['name']} / "
        ...
        approve_proxy = ""
        if submission.get("state") == "requires_approval":
            log("BLIK confirm memerlukan ChatGPT approve...")
            ...
            log("BLIK approve berhasil, menunggu status Stripe selanjutnya...")
        ...
        log("BLIK Konfirmasi Status Lanjutan Selesai")
    ...
    if env_bool("IDEAL_CONFIRM_INLINE_PM", False):
        log(
            f"Data inline iDEAL konfirmasi: {billing['name']} / "
            f"{billing['line1']} / {billing['city']} {billing['postal_code']} / bank={os.environ.get('IDEAL_BANK', '') or 'auto'}"
        )
    else:
        log(f"Buat PM (IDEAL): {billing['country']} {billing['name']} / {billing['city']}")
        pm_id = stripe_create_ideal_pm(stripe, checkout["cs_id"], stripe_pk, billing, ctx)
        log(f"PM berhasil dibuat: {pm_id}")
        log(
            f"Data Belanda sudah diisi ke PM billing_details: {billing['name']} / "
            f"{billing['line1']} / {billing['city']} {billing['postal_code']}"
        )

    if env_bool("IDEAL_UPDATE_CUSTOMER_DATA", False):
        log(
            f"Submit isian data Belanda: {billing['name']} / {billing['line1']} / "
    ...
    log("Stripe confirm berhasil, parsing redirect...")
    ...
    if redirect_url:
        log(f"konfirmasi URL pindai/otorisasi akhir diekstrak: {redirect_url[:180]}")
    if qr_urls:
        log(f"konfirmasi mengekstrak kandidat QR {len(qr_urls)} buah")
    if stop_event and stop_event.is_set():
        raise RuntimeError("Tugas telah dihentikan, lewati putaran ini")

    if not redirect_url and submission.get("state") == "requires_approval":
        log("Membutuhkan persetujuan ChatGPT...")
        approve_preferences = successful_approve_preferences(checkout_proxy, provider_proxy, [checkout_proxy] + approve_pool)
        if approve_preferences:
            log(f"Hit proxy sukses approve diprioritaskan: {proxy_label(approve_preferences[0])}")
        approve_proxies = list(dict.fromkeys(approve_preferences + [checkout_proxy]))
        log("Perlu approve: skenario iDEAL 0 nilai, approve gunakan ulang proxy checkout saat ini, ganti ke percobaan berikutnya jika gagal.")
        approve_proxy = approve_with_retry(access_token, device_id, checkout, approve_proxies, session_token, "checkout")
        log("Mengikuti redirect untuk mengekstrak chain akhir...")
        redirect_url, poll_qr = poll_payment_page(stripe, checkout, stripe_pk, ctx)
        qr_urls.extend(poll_qr)
        if redirect_url:
            record_proxy_pair_approve_success(checkout_proxy, provider_proxy, approve_proxy)
            log("Selesai - Combo checkout/provider/approve ini telah diingat")
    elif not redirect_url and not qr_urls:
        log("konfirmasi tidak mengembalikan redirect/QR iDEAL asli, lanjut poll payment_pages untuk verifikasi akhir", "[WARN] ")
        redirect_url, poll_qr = poll_payment_page(stripe, checkout, stripe_pk, ctx)
        qr_urls.extend(poll_qr)
    ...
    if redirect_url:
        ...
        if final_url and final_url != redirect_url:
            log(f"Mengikuti redirect mendapatkan chain akhir: {final_url[:180]}")
...
def run_once(
    ...
    expected_label = payment_method_label()
    log(f"Mulai ekstraksi {expected_label}, percobaan ke {attempt}/{max_retry}")
    log(
        "Tes kombinasi: "
    ...
    log(f"Proxy ronde ini: checkout/kualifikasi={proxy_label(checkout_proxy)}；Stripe/{expected_label}={proxy_label(provider_proxy)}")
    ...
    if zero_status == "ok":
        log(f"cache hit kualifikasi checkout Rp 0: amount={zero_amount}")
    elif zero_status == "bad":
        log(f"cache hit checkout gagal Rp 0: amount sebelumnya={zero_amount}, putaran ini lanjut verifikasi", "[WARN] ")
    if stop_event and stop_event.is_set():
        raise RuntimeError("Tugas telah dihentikan, lewati putaran ini")
    ...
    try:
        ...
        if is_user_already_paid_error(exc):
            raise RuntimeError("Pengguna telah membayar: User is already paid") from exc
        if is_checkout_not_active_error(exc):
            raise
...
def run_attempt(
    ...
    if batch_no > 0:
        _log_context.prefix = f"[Batch {batch_no}/{batch_total or '?'}][Putaran {attempt}/{max_retry}] "
    else:
        _log_context.prefix = f"[Putaran {attempt}/{max_retry}] "
    try:
        ...
        error = str(exc)
        if error.startswith("Tugas Telah Dihentikan"):
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, ""
        if is_user_already_paid_error(error):
            log("Terdeteksi User is already paid: pengguna telah membayar, hentikan tugas")
            if stop_event:
                stop_event.set()
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, error
        if "当前 checkout 不支持" in error:
            log(f"Percobaan {attempt}/{max_retry} gagal: {error[:300]}, terus mencoba proxy/checkout berikutnya", "[WARN] ")
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, error
        if is_checkout_not_active_error(error):
            log(
                f"Sesi putaran ke-{attempt}/{max_retry} telah kadaluarsa; lewati putaran ini, jangan catat kegagalan proxy, proxy disimpan untuk penggunaan berikutnya",
                "[WARN] ",
            )
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, error
        record_failure_by_stage(error, checkout_proxy, provider_proxy)
        log(f"Gagal pada putaran ke-{attempt}/{max_retry}: {error[:300]}", "[WARN] ")
        _log_context.prefix = previous_log_context
        return attempt, "", [], checkout_proxy, provider_proxy, error
...
def build_attempt_batches(checkout_proxies: list[str], provider_proxies: list[str], max_attempts: int) -> list[tuple[str, list[str]]]:
    ...
    for checkout_proxy in checkout_proxies:
        ...
        if attempt_count >= max_attempts:
            break
    if preferred_count:
        log(f"Penjadwalan memprioritaskan kombinasi sukses: {preferred_count} grup")
    return batches
...
def run_single_link_attempt(
    ...
    try:
        if stop_event.is_set():
            return attempt, "", "Tugas telah dihentikan, lewati putaran ini", False
        billing = ideal_billing_profile()
        ...
        if not provider_candidates:
            log("Semua proxy Provider telah disaring berdasarkan status gagal, hentikan ekstraksi tautan kali ini", "[WARN] ")
            return attempt, "", "provider_all_filtered", False
        checkout: dict[str, str] | None = None

        log(f"Mulai link ke-{attempt}/{ideal_retry}")
        log(
            f"Langkah 1: Buat checkout ChatGPT... tagihan checkout={checkout_country}/{checkout_currency},"
            f"Ke-{attempt}/{ideal_retry}, setiap putar acak ambil maks {checkout_retry} node"
        )
        log(f"Negara PM pertama: {pm_country}")
        ...
        for checkout_index, checkout_proxy in enumerate(checkout_candidates, start=1):
            ...
            try:
                ...
                log(f"Checkout {checkout_index}/{len(checkout_candidates)} gagal: {error[:220]}", "[WARN] ")
        ...
        if preferred_providers:
            ...
            log(f"Prioritaskan kombinasi checkout/provider yang berhasil: provider={len(preferred_providers)}")

        stripe_pk = checkout.get("stripe_pk") or DEFAULT_STRIPE_PK
        log("Stripe publishable key loaded")
        log(f"Langkah 2: Coba pertama PM={pm_country}...")
        ...
        for provider_index, provider_proxy in enumerate(provider_candidates, start=1):
            ...
            _log_context.prefix = f"[{expected_label} {attempt}/{ideal_retry}][PM={pm_country}] "
            try:
                log(f"Provider(acak {provider_index}/{len(provider_candidates)}): {proxy_label(provider_proxy)}")
                ensure_proxy_country("provider", provider_proxy)
                ensure_proxy_targets("provider", provider_proxy)
                ...
                last_error = error
                if is_checkout_not_active_error(error):
                    log("Sesi tidak berlaku; checkout saat ini tidak lanjut ganti provider", "[WARN] ")
                    break
                record_failure_by_stage(error, checkout_proxy_used, provider_proxy)
                log(f"Provider {provider_index}/{len(provider_candidates)} gagal: {error[:220]}", "[WARN] ")
                if is_approve_failure_error(error):
                    if "approve blocked" in error:
                        approve_blocked = True
                    log("Gagal approve, grup checkout/approve saat ini gagal, lanjut ke percobaan berikutnya", "[WARN] ")
                    break
                if "当前 checkout 不支持" in error:
                    log(f"Provider saat ini tidak mendapatkan {expected_label}, coba provider berikutnya", "[WARN] ")
                    continue
                if "0 元优惠未生效" in error:
                    break

        _log_context.prefix = f"[{expected_label} {attempt}/{ideal_retry}] "
        log(f"Link ke-{attempt}/{ideal_retry} selesai, tidak dapat URL akhir", "[WARN] ")
...
def run_single_link_parallel_mode(access_token: str, session_token: str, checkout_proxies: list[str], provider_proxies: list[str]) -> int:
    ...
    if requested_workers > workers:
        log(f"{expected_label} konkurensi dibatasi dari {requested_workers} menjadi {workers}", "[WARN] ")
    log(
        f"Mulai proses ekstraksi rantai {expected_label}:"
        f"checkout={checkout_country}/{checkout_currency}，PM={provider_country_label()}，locale={payment_browser_locale()}，"
        f"Checkout coba ulang={checkout_retry}, Provider coba ulang={provider_retry}, total coba ulang {expected_label}={ideal_retry}, konkurensi {expected_label}={workers}."
    )
    ...
    try:
        ...
        for future in as_completed(futures):
            try:
                ...
                error = str(exc)
                approve_blocked = False
                log(f"Percobaan ke-{attempt}/{ideal_retry} untuk ambil tautan tidak normal: {error[:300]}", "[WARN] ")
            if redirect_url:
                stop_event.set()
                for pending in futures:
                    pending.cancel()
            ...
            if is_user_already_paid_error(error):
                log("Terdeteksi User is already paid: pengguna telah membayar, tugas selesai normal")
                stop_event.set()
                for pending in futures:
                    pending.cancel()
                return 0
            if approve_blocked:
                ...
                log(f"Hitungan approve terblokir: {approve_blocked_count}/{max_blocked}", "[WARN] ")
            if approve_blocked_count >= max_blocked:
                log("Mencapai batas atas approve terblokir akun saat ini, berhenti ajukan link baru", "[WARN] ")
                stop_event.set()
                for pending in futures:
                    pending.cancel()
                return 1
    ...
    log(f"Semua gagal: {last_error}", "[ERROR] ")
    return 1
...
def run_single_link_mode(access_token: str, session_token: str, checkout_proxies: list[str], provider_proxies: list[str]) -> int:
    ...
    log(
        f"Mulai proses ekstraksi rantai {expected_label}:"
        f"checkout={checkout_country}/{checkout_currency}，PM={provider_country_label()}，locale={payment_browser_locale()}，"
        f"Checkout coba ulang={checkout_retry}, Provider coba ulang={provider_retry}, total coba ulang {expected_label}={ideal_retry}."
    )
    ...
    for attempt in range(1, ideal_retry + 1):
        ...
        provider_candidates = pick_random_proxies(provider_proxies, provider_retry, "provider")
        if not provider_candidates:
            last_error = "Semua Provider proxy telah disaring berdasarkan status gagal"
            log(last_error, "[WARN] ")
            break
        checkout: dict[str, str] | None = None
        checkout_proxy_used = ""
...
def run_single_seed_mode(access_token: str, session_token: str, proxy_seeds: list[str]) -> int:
    ...
    log(
        "Mulai eksekusi alur Seed tunggal BLIK:"
        f"PL checkout -> PL Stripe/BLIK -> PL approve；"
        f"Kandidat per ronde={checkout_retry}, total ulang={max_retry}."
    )
    for attempt in range(1, max_retry + 1):
        ...
        if not seed_candidates:
            last_error = last_error or "Semua Proxy Seed untuk tugas ini telah dicoba"
            log("Semua Proxy Seed untuk tugas ini telah dicoba, node gagal tidak diulang", "[WARN] ")
            break

        log(f"Mulai kirim BLIK percobaan ke {attempt}/{max_retry}")
        for seed_index, proxy_seed in enumerate(seed_candidates, start=1):
            ...
            try:
                ...
                log(
                    f"Seed {seed_index}/{len(seed_candidates)}: Seed={len(attempted_seed_keys)} telah dicoba kali ini"
                )
                ...
                log("BLIK tidak mengembalikan status selesai, coba Seed berikutnya yang belum dicoba", "[WARN] ")
            except Exception as exc:
                error = str(exc)
                last_error = error
                if is_user_already_paid_error(error):
                ...
                if is_checkout_not_active_error(error):
                    log("Session telah kedaluwarsa; Seed saat ini tidak mencatat cooldown, tetapi tugas ini tidak akan mengekstrak ulang", "[WARN] ")
                elif "当前 checkout 不支持" in error:
                    log("Checkout saat ini tidak menyediakan BLIK, coba Seed berikutnya yang belum dicoba", "[WARN] ")
                else:
                    log(f"Seed {seed_index}/{len(seed_candidates)} gagal: {error[:220]}", "[WARN] ")
            ...
            if approve_blocked_count >= max_blocked:
                log("Mencapai batas maksimum approve blocked untuk akun saat ini, hentikan pengajuan lebih lanjut", "[WARN] ")
                return 1

        log(f"Percobaan {attempt}/{max_retry} pengiriman BLIK berakhir, tidak selesai", "[WARN] ")
...
def run_legacy_two_pool_mode() -> int:
    ...
    if raw_max_retry:
        ...
        log(f"IDEAL_MAX_RETRY tidak diatur, default {per_checkout} percobaan per batch, maksimal 30 batch: total {max_retry} kali")
    ...
    if max_retry <= 0:
        log("Proxy kosong, tidak dapat memulai", "[ERROR] ")
        return 1

    max_blocked = env_int("IDEAL_MAX_APPROVE_BLOCKED", max_retry)
    expected_label = payment_method_label()
    checkout_proxy_country = expected_proxy_country("checkout")
    log(f"Strategi penjadwalan: setiap batch 1 checkout/{checkout_proxy_country} dipasangkan dengan {per_checkout} provider acak; ganti batch jika seluruh batch gagal; batas blocked approve={max_blocked}")
    ...
    if requested_workers > workers:
        log(f"Konkurensi alur formal dibatasi dari {requested_workers} menjadi {workers}", "[WARN] ")
    log(f"Konkurensi alur formal: pekerja per batch={workers}, batches={len(attempt_batches)}, percobaan={max_retry}")
    ...
    for batch_no, (checkout_proxy, provider_batch) in enumerate(attempt_batches, start=1):
        ...
        log(
            f"Mulai batch ke {batch_no}/{len(attempt_batches)}: "
            f"checkout/{checkout_proxy_country}={proxy_label(checkout_proxy)}，provider={len(provider_batch)}，konkurensi={batch_workers}"
        )
        ...
        try:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    redirect_url = ""
                    error = str(exc)
                    log(f"Percobaan {attempt}/{max_retry} pengecualian: {error[:300]}", "[WARN] ")
                if redirect_url:
                ...
                if "当前 checkout 不支持" in (error or ""):
                    log(f"Percobaan {attempt}/{max_retry} tidak menyediakan {expected_label}, terus mencoba proxy/checkout berikutnya", "[WARN] ")
                    continue
                if "approve blocked" in (error or ""):
                    approve_blocked_count += 1
                    log(f"Hitungan approve terblokir: {approve_blocked_count}/{max_blocked}", "[WARN] ")
                if approve_blocked_count >= max_blocked:
                    log("Mencapai batas maksimum approve blocked untuk akun saat ini, hentikan pengajuan batch baru", "[WARN] ")
                    stop_event.set()
                    break
        finally:
            executor.shutdown(wait=True, cancel_futures=stop_event.is_set())
        ...
        if approve_blocked_count >= max_blocked:
            return 1
        log(f"Batch {batch_no}/{len(attempt_batches)} selesai, belum mendapat URL akhir", "[WARN] ")
...
def main() -> int:
    ...
    if flow_mode != "single":
        log(f"IDEAL_FLOW_MODE={flow_mode} telah konvergen ke jalur Seed Tunggal BLIK", "[WARN] ")
