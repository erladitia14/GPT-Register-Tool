...
async def _do_pay(
    ...
    try:
        ...
        el = await page.find("Buat Akun", timeout=5)
        if el:
            await el.click()
            create_clicked = True
            print("[nd-pay] Mengklik Buat akun")
    except Exception:
        pass
    ...
    await _click_button(page, [
        "halaman berikutnya", "Next", "Langkah berikutnya", "Continue", "Continue with email",
    ])
    await asyncio.sleep(3)
    ...
    await _click_button(page, [
        "Agree & Pay", "Agree and Pay", "Setuju & Bayar", "Setuju & Bayar",
        "Pay", "Pay Now", "Submit", "Konfirmasi",
        "Setuju & Lanjutkan", "Continue", "halaman berikutnya",
    ])
    await asyncio.sleep(5)
...
async def _handle_sms(page, phone: str, sms_cfg: dict, deadline: float) -> str | None:
    ...
    await _click_button(page, [
        "Kirim Kode Verifikasi", "Send Code", "Dapatkan kode verifikasi", "Kirim", "Send",
        "Send SMS", "Kirim SMS", "Send verification code",
    ])
    await asyncio.sleep(2)
