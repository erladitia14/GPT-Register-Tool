...
def _recover_remail_service_token(mailbox, proxy=None):
    ...
    if mode == "code" and (not current_token or status in {"completed", "closed", "refunded", "failed"}):
        deadline = f"(Batas penerimaan {receive_until})" if receive_until else ""
        raise RuntimeError(
            f"Pesanan SMS sekali pakai ReMail telah kedaluwarsa{deadline}. API Key hanya dapat menanyakan pesanan, tidak dapat menggantikan Service Token "
            "Membaca inbox yang kedaluwarsa; buat pesanan baru, untuk melihat jangka panjang gunakan mode purchase."
        )
    raise RuntimeError(
