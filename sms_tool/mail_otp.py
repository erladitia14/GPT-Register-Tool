...
OTP_CONTEXT_MARKERS = ("code", "verification", "verify", "openai", "chatgpt", "login", "Kode verifikasi", "Kode verifikasi")
...
def _extract_otp_from_text(text):
    ...
    for match in OTP_RE.finditer(text):
        ...
        if re.search(r"(?i)" + re.escape(code) + r"\s*(px|em|rem|%)\b", context):
            continue
        score = 0
        if any(k in context_lc for k in ("code", "verification", "verify", "openai", "chatgpt", "login", "Kode verifikasi", "Kode verifikasi")):
            score += 10
