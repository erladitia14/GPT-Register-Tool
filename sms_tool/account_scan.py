...
def _scan_one(
    ...
    if oauth_result.get("ok"):
        ...
        result["phone_verification_required_label"] = "Ya" if result.get("phone_verification_required") else "Tidak"
        result["dropped"] = "Tidak"
        _persist_scan(data, json_path, result)
    ...
    if phone_required:
        ...
        result["subscription_type"] = _subscription_type(data)
        result["at_status"] = _at_status_label(result, refresh_result, oauth_result)
        result["phone_verification_required_label"] = "Ya"
        result["dropped"] = "Tidak"
        _persist_scan(data, json_path, result)
...
def _scan_overview(result):
    ...
    dropped = (
        ...
        or dropped_value in {"Ya", "yes", "true", "1"}
    )
    return {
        ...
        "phone_verification_required": "Ya" if bool((result or {}).get("phone_verification_required")) else "Tidak",
        "subscription_type": _subscription_type(result or {}),
        "dropped": "Ya" if dropped else "Tidak",
    }
...
def _print_scan_overview(results):
    if not results:
        return
    print("[*] Ringkasan pemindaian akun:")
    for index, result in enumerate(results, 1):
        overview = _scan_overview(result)
        print(
            f"{index}. {overview['email']} | "
            f"AT: {overview['at_status']} | "
            f"Perlu verifikasi nomor ponsel: {overview['phone_verification_required']} | "
            f"Tipe langganan: {overview['subscription_type']} | "
            f"Akun jatuh: {overview['dropped']}"
        )
