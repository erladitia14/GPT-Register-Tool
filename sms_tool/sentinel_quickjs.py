...
def _ensure_sdk_file(session: Any, timeout_ms: int) -> Path:
    ...
    if status != 200:
        ...
        if status in (403, 404):
            hint = (
                f"(Versi Sentinel {version} mungkin telah diganti/dinonaktifkan oleh OpenAI,"
                "Perbarui variabel lingkungan OPENAI_SENTINEL_VERSION atau sentinel_version di config)"
            )
        raise RuntimeError(f"Gagal mengunduh sdk.js: HTTP {status}{hint}")
    content = getattr(resp, "content", b"") or (resp.text or "").encode()
    if not content:
        raise RuntimeError("Gagal mengunduh sdk.js: Respons kosong")
...
def _run_quickjs_action(
    *,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"Eksekusi QuickJS gagal: {(proc.stderr or proc.stdout or 'unknown').strip()[:300]}")
    out = (proc.stdout or "").strip()
    if not out:
        raise RuntimeError("QuickJS mengembalikan output kosong")
    data = json.loads(out)
    if not isinstance(data, dict):
        raise RuntimeError("Output QuickJS bukan objek JSON")
    return data
...
def _fetch_sentinel_challenge(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Response Sentinel challenge bukan objek JSON")
    return payload
...
def get_sentinel_token_via_quickjs(
    ...
    *,
    ...
    if not quickjs_script.exists():
        log(f"Skrip Sentinel QuickJS tidak ada: {quickjs_script}")
        return None
    ...
    try:
        ...
        if not request_p:
            log("Sentinel QuickJS gagal: requirements tidak mengembalikan request_p")
            return None
        ...
        if not c_value:
            log("Sentinel QuickJS gagal: challenge token kosong")
            return None
        ...
        if not final_p:
            log("Sentinel QuickJS gagal: solve tidak mengembalikan final_p")
            return None

        t_raw = solved.get("t")
        t_value = "" if t_raw is None else str(t_raw).strip()
        if not t_value:
            log("Sentinel QuickJS gagal: solve tidak mengembalikan t yang valid")
            return None
        ...
        log(f"Sentinel QuickJS berhasil (p_len={len(final_p)} t_len={len(t_value)} c_len={len(c_value)})")
        return token
    except Exception as e:
        log(f"Sentinel QuickJS mengalami pengecualian: {e}")
        return None
