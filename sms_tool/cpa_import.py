...
def refresh_cpa_quota_statuses(emails=None, workers=4, api_url="", api_token="", timeout=30):
    ...
    def _run(index, email, item):
        ...
        status = str(probe.get("quota_status") or probe.get("status") or "Tidak Dikenal")
...
def classify_cpa_auth_file(item):
    ...
    if (
        ...
        or re.search(r"\bbanned\b|\bsuspended\b|\bdeactivated\b|\bterminated\b|account closed|account_locked|fraud|abuse|violation|penangguhan|pelanggaran|diblokir", text)
        or probe_status == 401
...
def _build_cpa_payload(token_data):
    ...
    if not access_token:
        return {"ok": False, "error": "missing_access_token", "message": "Impor CPA kekurangan access_token."}
...
def _load_cpa_source(email="", session_file="", export_dir=""):
    ...
    return {
        ...
        "message": "Impor CPA memerlukan file JSON dengan access_token yang sudah ada; akun saat ini tidak ditemukan AT JSON yang dapat diimpor.",
        "path": json_path or session_file or "",
    }
