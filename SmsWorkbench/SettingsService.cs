        ...
        public SettingsSaveResult Save(IEnumerable<SettingsCategoryViewModel> categories)
        {
            ...
            foreach (SettingFieldViewModel field in fields.Where(field => field.Kind == SettingFieldKind.Number))
            {
                if (field.Value.Trim().Length > 0 && !int.TryParse(field.Value.Trim(), out _))
                    return new SettingsSaveResult(false, field.Label + " harus berupa bilangan bulat.");
            }
            ...
            try
            {
                ...
                if (matrix is not JsonObject)
                    return new SettingsSaveResult(false, "Node akar matriks kelayakan wilayah harus berupa objek JSON.");
            }
            catch (Exception exception)
            {
                return new SettingsSaveResult(false, "JSON matriks kelayakan wilayah tidak valid:" + exception.Message);
            }
            ...
            {
                return new SettingsSaveResult(false, "Penyimpanan konfigurasi gagal: " + exception.Message);
            }
        }
