...
public sealed class ProtocolPaymentExecutionPlannerTests
{
    ...
    public void PayPalAccountPlanPreservesSessionRefreshAndStageOptions()
    {
        ...
        Assert.Equal("Ekstraksi tautan protokol PayPal", plan.TaskName);
        Assert.Equal("Menjalankan ekstraksi tautan protokol PayPal...", plan.StatusText);
    ...
    public void BlikPlanUsesPaymentOperationOnlyOutsideProbe()
    {
        ...
        Assert.Equal("BLIK Pembayaran Protokol", plan.TaskName);
...
public sealed class ProtocolPaymentResultPresenterTests
{
    ...
    public void SuccessfulResultProducesDisplayTextAndCopyTargets()
    {
        ...
        Assert.Contains("[Sukses] Ekstraksi berhasil!", result.Text, StringComparison.Ordinal);
        Assert.Equal("extract_link", result.Operation);
        Assert.Equal("completed", result.TerminalState);
        Assert.Contains("AT Probe: HTTP 200", result.Text, StringComparison.Ordinal);
        Assert.Contains("Status Persetujuan: Disetujui", result.Text, StringComparison.Ordinal);
    }

    [Theory]
    [InlineData("unknown", "[Hasil tidak diketahui, silakan cek status akun dulu, jangan coba ulang]")]
    [InlineData("cancelled", "[Dibatalkan]")]
    [InlineData("timed_out", "[Timeout]")]
    public void TerminalFailureUsesExistingUiClassification(string state, string prefix)
    {
        ...
        Assert.Contains("Kode Error: E_STOP", result.Text, StringComparison.Ordinal);
    ...
    public void CompletedExecutePaymentIsShownAsPaymentOnlyWhenOperationSaysSo()
    {
        ...
        Assert.Contains("[Sukses] Pembayaran selesai", result.Text, StringComparison.Ordinal);
    ...
    public void AbortingSideEffectingPlanRequiresReconciliation()
    {
        ...
        Assert.Contains("Jangan ulangi", result.Text, StringComparison.Ordinal);
    }
